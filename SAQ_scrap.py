...
def build_saq_url(
    ...
    if category:
        full_url = urljoin(base_url, f"{category}?{query_string}" if query_string else category)
    else:
...
def fetch_wine_details(url):
    try:
        ...
        for product in product_elements:
            ...
            volume_country = details.find('div', class_='product-item-attribute') if details else None
            if volume_country:
                volume_country_text = volume_country.get_text(strip=True)
                volume, country = volume_country_text.split('|')[0].strip(), volume_country_text.split('|')[-1].strip()
            else:
